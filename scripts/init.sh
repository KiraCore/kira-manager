...
ESSENTIALS[8]="cd $HOME/tmp/kira-manager && git checkout origin/feature/tinyinit >> ../log && make >> ../log && make init >> ../log"
ESSENTIALS[9]="sudo -S <<< \"$PSWD\" cp restserver /usr/local/bin && sudo chown '$WHOAMI':'$WHOAMI' /usr/local/bin/restserver && echo 'Done' >> ../log" 
...
whiptail --title "KM2.0 Setup" --msgbox "Installation completed. KM2.0 status can be found at localhost:8080/status" 10 60 && clear
exit 0
